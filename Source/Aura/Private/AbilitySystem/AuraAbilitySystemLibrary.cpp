...
{
	const bool bFirstIsPlayer = FirstActor->ActorHasTag(ACTOR_TAG_PLAYER);
	const bool bSecondIsPlayer = SecondActor->ActorHasTag(ACTOR_TAG_PLAYER);
	const bool bFirstIsEnemy = FirstActor->ActorHasTag(ACTOR_TAG_ENEMY);
	const bool bSecondIsEnemy = SecondActor->ActorHasTag(ACTOR_TAG_ENEMY);

	const bool bBothPlayers = bFirstIsPlayer && bSecondIsPlayer;
	const bool bBothEnemies = bFirstIsEnemy && bSecondIsEnemy;
	...
	const FDamageEffectParams& DamageEfeEffectParams)
{
	...
	SetDeathImpulse(EffectContextHandle, DamageEfeEffectParams.DeathImpulse);
	SetKnockbackForce(EffectContextHandle, DamageEfeEffectParams.KnockbackForce);

	SetIsRadialDamage(EffectContextHandle, DamageEfeEffectParams.bIsRadialDamage);
	SetRadialDamageInnerRadius(EffectContextHandle, DamageEfeEffectParams.RadialDamageInnerRadius);
	SetRadialDamageOuterRadius(EffectContextHandle, DamageEfeEffectParams.RadialDamageOuterRadius);
	SetRadialDamageOrigin(EffectContextHandle, DamageEfeEffectParams.RadialDamageOrigin);
...
void UAuraAbilitySystemLibrary::SetIsRadialDamageEffectParams(FDamageEffectParams& DamageEffectParams,
                                                              bool bIsRadialDamageEffect, float InnerRadius, float OuterRadius, FVector Origin)
{
                                                      ...
                                                      FVector KnockbackDirection, float Magnitude)
{
	...
	DamageEffectParams.KnockbackForce = KnockbackDirection * (Magnitude != 0.0f ? Magnitude : DamageEffectParams.KnockbackForceMagnitude);
}
                                                         ...
                                                         FVector ImpulseDirection, float Magnitude)
{
	...
	DamageEffectParams.DeathImpulse = ImpulseDirection * (Magnitude != 0.0f ? Magnitude : DamageEffectParams.DeathImpulseMagnitude);
}
