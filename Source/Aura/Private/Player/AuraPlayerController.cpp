...
void AAuraPlayerController::CursorTrace()
{
	...
	const ECollisionChannel TraceChannel = IsValid(MagicCircle) ? ECC_ExcludePlayers : ECollisionChannel::ECC_Visibility;
	 ...
	 *	D. Both actors arent null, but LastActor != ThisActor
	 *		- UnHighlight LastActor, Highlight ThisActor
	 *	E. Both actors arent null, but LastActor == ThisActor
	 *		- Do nothing
	 */
		...
		if (FollowTime <= ShortPressThreshold && ControlledPawn)
		{
			if (UNavigationPath* NavPath = UNavigationSystemV1::FindPathToLocationSynchronously(this, ControlledPawn->GetActorLocation(), CachedDestination))
			{
				...
				for (const FVector& PointLoc : NavPath->PathPoints)
				{
					...
					// DrawDebugSphere(GetWorld(), PointLoc, 32, 8, FColor::Green, false, 5.0f);
				}
